...
- [x] Join / invite flows → `GET` snapshot + `PUT` seat before navigating to waiting/play
- [x] Waiting room → poll or fetch `GET /api/v1/room/{id}`; show open team slots / `canJoin`
- [x] Host dispose (if exposed) → `DELETE /api/v1/room/{id}`
...
- [x] `RemotePlayer` component (skin from `getSoldierSkinById`; hitboxes from combat preset when needed)
- [x] Wire local FPS transform sync through adapter (not direct Colyseus in `game/`)
- [x] Wire `useShooting` → `sendShot`; apply **server** HP / eliminated updates (client combat math becomes prediction-only or removed for HP)
- [x] Init Colyseus join on waiting room and/or `/room/{id}/play` mount (`joinById` / reserved seat)
- [x] Lock joins when `roundPhase` leaves `waiting`

## Verification

- [x] Two browsers: create via REST, join via PUT + WebSocket, teams ≤4 each, shoot until one team eliminated; host Restart starts next round; Home returns to `/`
- [x] Status reflects `canJoin` false when full or `in_progress`; PUT returns `409` when team full
