...
- `LocalPlayer.tsx` — one clone + mixer; FPS hides the head; look pitch bends the spine in **all** camera modes so arms follow the mouse
...
**FPS (one mesh):** after the mixer, place the camera at the head bone’s world position (not a fixed `PLAYER_EYE_HEIGHT` clip hack). Scale Head to zero so the camera is not inside the helmet. Apply look pitch to spine/upper-body bones in **every** camera mode so arms follow the mouse over the shoulder and behind the back too; only the head hide is FPS-only. Rig yaw stays as today. No second clone / `FpsViewModel`.

Bone/asset contract (`soldiers/utils/aim-body-rig.ts`): Mixamo bones appear as `mixamorig:<Name>` or `mixamorig<Name>` — lookups try both spellings (GLTFLoader sanitizes `:` out of node names). Camera anchor = `Head`; spine chain `Spine` → `Spine1` → `Spine2` receives look pitch (weights 0.25 / 0.35 / 0.4, premultiplied onto the clip pose) in all camera modes; the head hide/restore is FPS-only. The clips animate `Head`/`Neck` **scale** channels every frame, and three's mixer never overwrites externally mutated bone properties once touched (cached PropertyMixer values), so both the hide (FPS) and the restore (shoulder modes) are re-enforced after each mixer update — and the same rule governs pitch: bones whose quaternion the mixer refreshed get the full look pitch re-applied, while bones frozen by a clip without spine rotation channels (e.g. clamped kneel) only take the pitch **delta** since the previous frame; applying full pitch to a frozen bone would spin the body every frame. Neck stays visible on purpose: scaling a parent bone to zero collapses its children’s translation, which would pull the head anchor down to the neck base. If looking straight down still exposes chest interior, hide further meshes/bones but never the whole body. Frame order: `PlayerControls` runs first (OTS/TPS placement + pre-mount FPS fallback); `LocalPlayer`’s post-mixer callback applies body pose and overrides the camera with head-bone placement while FPS is active. Skinned meshes skip frustum culling: per-frame raycasts run before matrix updates and can cache a degenerate bounding sphere that culls the whole soldier.

**Shoulder cameras ride the body:** `LocalPlayer` writes the head-bone world Y into shared player state (`getBodyAnchorY`) after each mixer update, and `applyCameraMode` pivots the OTS/TPS boom height on that anchor instead of a fixed ground height — kneeling drops and jumping raises the camera with the soldier. Placement runs one frame behind the anchor (controls fire before the mixer update), which reads as smooth follow.
