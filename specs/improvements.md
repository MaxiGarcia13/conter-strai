...
- [ ] **Split scenario textures by phase** — Replace single `getScenarioTextureIds(scenario)` / `useScenarioTextureLibrary` call with phase-scoped helpers (e.g. ground, houses). Each phase gets its own `useLoader` subset so Suspense can resolve independently. Arena-01 data is already authored in separate files (`ground.ts`, `houses.ts`, `greenery.ts`, `infrastructure.ts`); keep `floorZones` split at compose time instead of merging early.
- [ ] **Staged `ScenarioScene`** — Split `scenario-scene.tsx` into phased sub-scenes (e.g. `ScenarioGround`, `ScenarioHouses`, `ScenarioProps`) each in its own Suspense boundary inside `game-canvas.tsx`. Props may keep preload for cache warmth but should mount only in the decorations phase.
