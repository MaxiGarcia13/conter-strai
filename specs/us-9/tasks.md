...
- [x] Re-engage look on pause **Resume** (Escape toggles pause; resume re-requests pointer lock)
- [x] E2e: `camera.spec.ts` — no pre-click for camera cycle

## Pause menu (US-9.3–US-9.7)

- [x] Add `src/modules/game/state/game-pause-store.ts`
- [x] Add `src/modules/game/constants/game-bindings.ts` (runtime codes + pause Commands list)
- [x] Add `src/modules/game/components/game-pause-panel.tsx`; mount in `game-canvas.tsx`
- [x] Escape toggles pause in `use-player-keyboard.ts` (**live** only)
- [x] Gate movement, look, shoot, pose actions when paused (`isPausedRef` in control hooks)
- [x] Pause open → `exitPointerLock()`; resume → re-request lock
- [x] Extract `src/modules/game/utils/restart-round.ts`; reuse in `round-end-banner.tsx` + pause panel
- [x] Add `src/modules/game/utils/leave-match-to-home.ts` (no `deleteRoom`)
- [x] Wire Restart (host only), Leave → `/`, Commands toggle, Resume
- [x] Reset pause store on round end / navigate away
- [x] Unit: pause store + commands list
- [x] E2e: `tests/e2e/play/pause-menu.spec.ts`
