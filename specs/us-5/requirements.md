...
| US-5.3  | US-7 lobby pages (`/room`, `/room/{id}`, `/room/{id}/join`, `/room/{id}/play`) call REST for create / snapshot / seat; waiting room and play connect via Colyseus adapter (`joinById` / reserved seat) |
...
| US-5.12 | `GET /api/v1/room/{roomId}` returns `RoomSnapshot` (`phase`, `canJoin`, per-team seats `max: 4`, optional `scenario` / `playerCount`); `404` if unknown/disposed                                       |
