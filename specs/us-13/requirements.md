...
| US-13.3  | When the magazine is **empty** (`shotsInMag >= 12`), `fireWeapon()` returns without firing (same gating family as cooldown / reload pose). Plays `empty-gun.m4a` on dry-fire (respecting fire cooldown). **No auto-reload** — the player must reload manually.                                                                                                                                              |
| US-13.4  | **R** (desktop) triggers the existing reload pose when idle (`reloading`) or kneeling (`reloading-kneel`). Reload clips are **3 s**; plays `reloading-gun.m4a` near the end of the animation (local only). WASD cancel stops pending/active reload SFX. On mixer `finished`, the magazine refills to **12 available shots**. Voluntary reload while the magazine is not empty is allowed (tactical reload). |
...
| US-13.8  | When a shot's raycast hits **visible world geometry** (walls, floor, props — untagged meshes) within **~10 m** of the camera, spawn a **client-only black impact mark** at the hit point (bullet-hole style). Marks are cosmetic; not synced to peers.                                                                                                                                                      |
...
1. Solo `/room/{id}/play` (desktop): fire 12 times — 13th click plays empty-gun SFX (no hitscan; respects fire cooldown). Press **R** while idle — 3 s reload animation; reload-gun SFX plays near the end; after it finishes, can fire 12 more times.
2. Reload while kneeling uses `reloading-kneel`; returns to kneel on finish; magazine refills.
3. WASD during standing reload cancels reload (existing behavior); reload SFX stops; magazine does **not** refill until a reload completes.
4. Touch-primary: reload button visible; tap reload when idle/kneel refills magazine; fire button blocked when empty.
5. Point-blank wall shot (≤ ~10 m): black mark appears on the surface; shot beyond 10 m at a wall leaves no mark.
