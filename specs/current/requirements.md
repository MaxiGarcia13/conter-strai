...
| ID    | Requirement                                                                                                                                                                                                                                                                                                                                            |
| ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| FR-1  | Landing at `/` (`src/pages/index.astro`) shows title **Conter Strai**, tagline, `cs.png`, game info (online PvP, **Civilians vs Soldiers**, round-based), **Start Game** → `/play`, footer **Contribute on GitHub** (repo from `package.json`), SEO meta, and a **dark tactical shooter theme** (amber accent, CSS atmosphere, no Three.js on landing) |
...
| FR-4  | Character skins are data-driven (`SoldierSkinId`); default play skin **`swat-1`**. Civilians: `remy` / `james` / `liza`. Soldiers: `swat-1` / `swat-2` / `swat-3`. Clips load from shared [`base-animations.glb`](../../public/assets/characters/shared/base-animations.glb)                                                                           |
...
| FR-12 | Round end: when **one team is fully eliminated**, declare winner (banner). Next-round respawn / HP reset is **US-5** (no local auto-restart)                                                                                                                                                                                                           |
...
| FR-15 | Local soldier plays **idle** / **walk** / **run** (stand / WASD / WASD+Space); **F** jump one-shot (clears kneel first); **E** kneel toggle. While kneeling, WASD keeps kneel and plays **crouch-walking** at walk speed (Space run ignored)                                                                                                           |
