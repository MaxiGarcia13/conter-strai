...
| US-12.4  | **Shoot:** bottom-right fire button triggers the same hitscan path as LMB (`fireWeapon`) with existing gating (pause, phase, pose, cooldown). Pointer-lock is **not** required on touch-primary.                                                                                                      |
...
| US-12.10 | Pause panel **Commands** list shows `MOBILE_BINDINGS` labels instead of keyboard keys when touch-primary: joystick for move, **A** (hold) sprint, **B** (tap) kneel, fire icon shoot, menu icon pause. Desktop list unchanged.                                                                                                                                 |
| US-12.11 | **Camera mode** can be changed from the main `GamePausePanel` menu (not only via **C**): shows the active mode label and a control that calls `cycleCameraMode()` (fps → ots → tps). Available on **touch-primary and desktop** while paused during **`live`**. No in-game camera button during play. |
...
2. Top-left pause opens `GamePausePanel`; Resume closes and restores controls; **Cycle camera** in pause menu changes fps / ots / tps and shows the active label.
...
- Jump, reload, and **in-game** camera-cycle on-screen buttons (camera changes via pause menu instead — US-12.11)
- Gamepad API
